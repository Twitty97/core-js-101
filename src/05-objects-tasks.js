...
class NewBuilder {
  ...
  element(s) {
    ...
    return new NewBuilder(this.a.concat([['element', s]]));
  }

  id(s) {
    if (this.a.some((e) => e[0] === 'id')) throw new Error(err2);
    return new NewBuilder(this.a.concat([['id', `#${s}`]]));
  }

  class(s) {
    return new NewBuilder(this.a.concat([['class', `.${s}`]]));
  }

  attr(s) {
    return new NewBuilder(this.a.concat([['attr', `[${s}]`]]));
  }

  pseudoClass(s) {
    return new NewBuilder(this.a.concat([['pseudoClass', `:${s}`]]));
  }

  pseudoElement(s) {
    if (this.a.some((e) => e[0] === 'pseudoElement')) throw new Error(err2);
    return new NewBuilder(this.a.concat([['pseudoElement', `::${s}`]]));
  }

  combine(s1, c, s2) {
    return new NewBuilder(this.a.concat(s1.a).concat([['element', ` ${c} `]]).concat(s2.a), false);
  }
...
const cssSelectorBuilder = new NewBuilder();
